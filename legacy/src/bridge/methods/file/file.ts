...
export const upload = (payload: FileUploadPayloadType): Promise<any> => {
    return new Promise((resolve, reject) => {
        if (bridge.platform === 'android') {
            unsubscribe.upload = bridge.subscribe('file.UploadCompleted', (event: any) => {
                unsubscribe.upload();
            ...
            bridge.dispatch('file.Upload', prepareBody(payload));
        } else {
...
export const uploadCompleted = (payload: FileUploadCompletedPayloadType): void => {
    bridge.broadcast('file.UploadCompleted', JSON.stringify(payload));
};
